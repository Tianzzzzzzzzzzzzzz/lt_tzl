...
public interface Compare<Children> extends Serializable {
    ...
    default <R> Children betweenIfPresent(SFunction<R, ?> column, Object val1, Object val2) {
        ...
        return between(false, column, val1, val2);
    }

    default <R> Children inIfPresent(SFunction<R, ?> column, Collection<?> values) {
        if (!CollectionUtils.isEmpty(values)) {
            return in(true,column, values);
        }
        return in(false,column, values);
    }
    default <R> Children inIfPresent(SFunction<R, ?> column, Collection<?> values,Object key) {
